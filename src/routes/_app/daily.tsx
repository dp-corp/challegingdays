...
const DEFAULT_HABITS = [
  ...
  ["Review goals", "morning"],
  ["Deep work session", "work"],
  ["Complete key work task", "work"],
  ["Business development activity", "work"],
  ["Learning session", "work"],
  ["Follow-up tasks", "work"],
  ["Walking", "personal"],
  ["Reading", "personal"],
  ["Social connection", "personal"],
  ["Journal entry", "personal"],
  ["Sleep early", "personal"],
] as const;
...
function DailyPage() {
  ...
  const projectsQ = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("projects").select("id,name,is_recurring")).data ?? [],
  });

  const logsQ = useQuery({
    queryKey: ["habit_logs", uid, date],
    queryFn: async () =>
      (await supabase.from("habit_logs").select("*").eq("user_id", uid).eq("log_date", date))
        .data ?? [],
  });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (habitsQ.data.length === 0) {
        const rows = DEFAULT_HABITS.map(([name, category], i) => ({
          user_id: uid,
          name,
          category,
          sort_order: i,
  ...
  const toggle = async (habitId: string, completed: boolean) => {
    ...
    qc.setQueryData(["habit_logs", uid, date], (prev: any) => {
      ...
      if (completed)
        return [
          ...arr.filter((l: any) => l.habit_id !== habitId),
    ...
    if (completed) {
      await supabase
        .from("habit_logs")
        .upsert(
          { user_id: uid, habit_id: habitId, log_date: date, completed: true },
    ...
    if (total > 0) {
      ...
      await supabase
        .from("scores")
        .upsert(
          { user_id: uid, score_date: date, habits_score: habitsScore, daily_score: habitsScore },
      ...
      const { data: profile } = await supabase
        ...
        .eq("id", uid)
        .maybeSingle();
      const dayNum = profile?.challenge_start_date
        ? challengeDay(profile.challenge_start_date)
        : undefined;
      const awarded = await maybeAwardBadges(
        uid,
        (allScores ?? []).map((s) => ({ date: s.score_date, score: s.daily_score })),
        dayNum,
      );
      if (awarded.length) {
  ...
  const recomputeProjectProgress = async (projectId: string) => {
    const { data: project } = await supabase
      .from("projects")
      .select("id,is_recurring,target_days,created_at")
      .eq("id", projectId)
      .maybeSingle();
    if (!project) return;
  ...
  const linkProject = async (id: string, projectId: string | null) => {
    await supabase
      .from("habits")
      .update({ project_id: projectId, category: projectId ? "project" : "custom" })
      .eq("id", id);
    qc.invalidateQueries({ queryKey: ["habits", uid] });
  ...
  const add = async () => {
    ...
    await supabase
      .from("habits")
      .insert({ user_id: uid, name: newName, category, project_id: projectId });
    setNewName("");
    setNewProject("none");
    qc.invalidateQueries({ queryKey: ["habits", uid] });
    if (projectId) recomputeProjectProgress(projectId);
  ...
  const consistency30 = useMemo(() => {
    if (!habitsQ.data || !allLogsQ.data || habitsQ.data.length === 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-dashed">
        <CardHeader>
          <CardTitle className="text-base">Add habit</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-wrap items-end gap-2">
          ...
          <Select value={newCat} onValueChange={setNewCat}>
            ...
            <SelectContent>
              {["morning", "work", "personal", "custom"].map((v) => (
...
function CategoryCard({
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <Card
        className={`overflow-hidden border-l-4 bg-gradient-to-br ${CATEGORY_ACCENT[cat]}`}
          ...
          <CardContent className="space-y-2 pt-0">
            {items.map((h: any) => (
              <HabitRow
                ...
                projects={projects}
                projectName={projectName}
                consistency={consistency30[h.id] ?? 0}
              />
            ))}
          </CardContent>
        </CollapsibleContent>
    ...
    <div className="flex items-center gap-2 rounded-lg border bg-card/70 px-3 py-2.5 hover:border-primary/40 hover:shadow-sm transition">
      ...
      {editing ? (
        ...
        <>
          <div className="flex-1 min-w-0">
            ...
            {habit.project_id && (
              <div className="text-[10px] text-accent truncate">
                ↳ {projectName(habit.project_id) ?? "Project"}
              </div>
            )}
          </div>
          ...
          {linking ? (
            <Select
              value={habit.project_id ?? "none"}
              onValueChange={(v) => {
                onLink(habit.id, v === "none" ? null : v);
                setLinking(false);
              }}
            >
          ...
          <Button size="icon" variant="ghost" onClick={() => onRemove(habit.id)}>
