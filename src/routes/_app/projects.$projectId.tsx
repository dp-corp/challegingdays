...
function ProjectDetail() {
  ...
  const projectQ = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("projects").select("*").eq("id", projectId).single()).data,
  });
  ...
  const habitLogsQ = useQuery({
    ...
    queryFn: async () => {
      if (habitIds.length === 0)
        return [] as { habit_id: string; log_date: string; user_id: string }[];
  ...
  const addHabit = async () => {
    ...
    await supabase
      .from("habits")
      .insert({ user_id: uid, name: habitName.trim(), category: "project", project_id: projectId });
    setHabitName("");
  ...
  const toggleHabitToday = async (habitId: string, completed: boolean) => {
    if (completed) {
      await supabase
        .from("habit_logs")
        .upsert(
          { user_id: uid, habit_id: habitId, log_date: today, completed: true },
  ...
  const per30 = useMemo(() => {
    ...
    for (const l of logs)
      if (new Date(l.log_date) >= from) m[l.habit_id] = (m[l.habit_id] ?? 0) + 1;
    return m;
  }, [logs]);

  const reportRangeDays = reportRange === "weekly" ? 6 : 29;
  const reportTitle = reportRange === "weekly" ? "Weekly report" : "Monthly report";
  const reportSubtitle = reportRange === "weekly" ? "last 7 days" : "last 30 days";

  // report: per-member completions in last 7 or 30 days
  const reportData = useMemo(() => {
    const since = subDays(new Date(), reportRangeDays);
    const counts: Record<string, number> = {};
    if (isRecurring) {
      for (const l of logs) {
        if (new Date(l.log_date) >= since) {
          counts[l.user_id] = (counts[l.user_id] ?? 0) + 1;
        }
      }
    } else {
      for (const t of tasks) {
        if (t.status === "completed" && new Date(t.updated_at) >= since) {
    ...
    return members
      .map((m) => ({
        id: m.id,
        name: m.display_name || (m.id === projectQ.data?.user_id ? "Owner" : "Member"),
        count: counts[m.id] ?? 0,
        isOwner: m.id === projectQ.data?.user_id,
      }))
      .sort((a, b) => b.count - a.count);
  }, [isRecurring, logs, tasks, members, projectQ.data?.user_id, reportRangeDays]);
  ...
  const toggleSharedMode = async (v: boolean) => {
    await supabase
      .from("projects")
      .update({ shared_completion: v } as any)
      .eq("id", projectId);
    qc.invalidateQueries({ queryKey: ["project", projectId] });
    toast.success(
      v
        ? "Shared mode: anyone ticking counts for the team"
        : "Personal mode: each member tracks their own",
    );
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="font-display text-3xl md:text-5xl bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent">
                {projectQ.data?.name}
              </h1>
              {isRecurring && (
                <span className="inline-flex items-center gap-1 text-xs uppercase tracking-widest text-primary bg-primary/10 px-2 py-1 rounded-full">
              ...
              <InviteButton
                projectId={projectId}
                ownerId={projectQ.data?.user_id}
                currentUid={uid}
              />
            </div>
            {projectQ.data?.description && (
              <p className="mt-2 text-muted-foreground max-w-2xl">{projectQ.data.description}</p>
            )}
          </div>
          {isRecurring ? (
            <div className="flex gap-2">
              <Input
                placeholder="New daily item…"
                value={habitName}
          ...
          <div className="flex justify-between text-sm">
            {isRecurring ? (
              <span className="text-muted-foreground">
                <span className="font-medium text-foreground">{daysDone}</span>/{targetDays} days
                done • <span className="text-accent">{daysLeft} left</span>
              </span>
            ) : (
              <span className="text-muted-foreground">
      ...
      {members.length > 1 && (isOwner || members.some((m) => m.id === uid)) && (
          ...
          <CardContent className="p-4 flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {sharedMode
                  ? "On — any member ticking an item counts for the whole team."
                  : "Off — each member tracks their own check-ins."}
              </p>
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Today's items
              {sharedMode && <span className="text-xs font-normal text-accent">(team)</span>}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {habits.length === 0 && (
              <p className="text-sm text-muted-foreground">
                No daily items yet. Add one above - it'll show on your Daily Tracker too.
              </p>
            )}
            {habits.map((h) => {
              ...
              return (
                <div
                  key={h.id}
                  className="flex items-center gap-3 rounded-lg border bg-card/60 px-3 py-2.5 hover:border-primary/40 transition"
                >
                      ...
                      {members.length > 1 && doneBy && doneBy.size > 0 && (
                        <span className="ml-1 text-accent">
                          ...
                          {Array.from(doneBy)
                            .map(
                              (id) =>
                                members.find((m) => m.id === id)?.display_name?.split(" ")[0] ||
                                "member",
                            )
                            .join(", ")}{" "}
                          done
                  ...
                  {isOwner && (
                    <Button size="icon" variant="ghost" onClick={() => removeHabit(h.id)}>
            ...
            <CardContent className="space-y-2">
              ...
              {tasks.map((t) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`text-sm ${t.status === "completed" ? "line-through text-muted-foreground" : ""}`}
                    >
                      {t.title}
                    </div>
                    {t.due_date && (
                  ...
                  <Button size="icon" variant="ghost" onClick={() => startEditTask(t)}>
              ...
              <div key={c.id} className="flex flex-col gap-2">
                ...
                <div className="space-y-2 rounded-xl border bg-card/30 p-2 min-h-32">
                  {tasks
                    ...
                    .map((t) => {
                      const daysRemaining = t.due_date
                        ? differenceInCalendarDays(new Date(t.due_date), new Date())
                        : null;
                      return (
                        <Card key={t.id}>
      ...
      {members.length > 1 && (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2 flex-wrap gap-2">
            <div>
              <CardTitle className="flex items-center gap-2 text-base">
                <UsersRound className="size-4 text-accent" />
                {reportTitle}
                <span className="text-xs font-normal text-muted-foreground">{reportSubtitle}</span>
              </CardTitle>
            </div>
            <div className="flex gap-1 border rounded-lg p-0.5 text-xs bg-muted/40">
              <button
                onClick={() => setReportRange("weekly")}
                className={`px-2.5 py-1 rounded-md transition ${reportRange === "weekly" ? "bg-card font-medium shadow-sm" : "hover:text-foreground/80"}`}
              >
                Weekly
              </button>
              <button
                onClick={() => setReportRange("monthly")}
                className={`px-2.5 py-1 rounded-md transition ${reportRange === "monthly" ? "bg-card font-medium shadow-sm" : "hover:text-foreground/80"}`}
              >
                Monthly
              </button>
            </div>
          ...
          <CardContent className="space-y-2">
            {reportData.map((w) => {
              const max = Math.max(1, ...reportData.map((x) => x.count));
            ...
            {reportData.every((w) => w.count === 0) && (
              <p className="text-sm text-muted-foreground">No completions yet in this period.</p>
            )}
          </CardContent>
        </Card>
      )}
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <DatePicker
                ...
                onChange={(v) => setEditForm({ ...editForm, due_date: v })}
              />
            </div>
            <div className="flex gap-2">
              <Button
...
function InviteButton({
  ...
  const [sharedCompletionOpt, setSharedCompletionOpt] = useState("true"); // "true" or "false"
  const qc = useQueryClient();

  const membersQ = useQuery({
    queryKey: ["project_members", projectId],
    queryFn: async () =>
      (await supabase.from("project_members" as any).select("user_id,role")).data ?? [],
  });
  const isOwner = ownerId === currentUid;
  const memberCount = (membersQ.data?.length ?? 0) + 1;

  const createInvite = async () => {
    // 1. Update project shared completion mode
    const isShared = sharedCompletionOpt === "true";
    await supabase
      .from("projects")
      .update({ shared_completion: isShared } as any)
      .eq("id", projectId);
    qc.invalidateQueries({ queryKey: ["project", projectId] });
    ...
    const { error } = await supabase
      ...
      .insert({ project_id: projectId, token, created_by: currentUid, expires_at: expires });
    if (error) return toast.error(error.message);
    const url = `${window.location.origin}/join/${token}`;
    try {
      await navigator.clipboard.writeText(url);
      toast.success("Style set and invite link copied!");
    } catch {
      toast.success("Style set and invite created", { description: url });
    }
    setInviteOpen(false);
  };
        ...
        <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Configure the checklist style before generating the invite link.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2">
              <Label className="text-sm font-medium">New member completion style:</Label>
              <div className="grid gap-2">
                <button
                  type="button"
                  onClick={() => setSharedCompletionOpt("true")}
                  className={`p-3 rounded-lg border text-left transition ${sharedCompletionOpt === "true" ? "border-primary bg-primary/5" : "hover:bg-muted/40"}`}
                >
                  <div className="font-semibold text-sm">Tick off for ALL (Shared Completion)</div>
                  <div className="text-xs text-muted-foreground mt-0.5">
                    Any team member can tick an item and it will count as done for everyone on the
                    team.
                  </div>
                </button>
                <button
                  type="button"
                  onClick={() => setSharedCompletionOpt("false")}
                  className={`p-3 rounded-lg border text-left transition ${sharedCompletionOpt === "false" ? "border-primary bg-primary/5" : "hover:bg-muted/40"}`}
                >
                  <div className="font-semibold text-sm">
                    Tick off for ONLY themselves (Personal Completion)
                  </div>
                  <div className="text-xs text-muted-foreground mt-0.5">
                    Each team member tracks and checks items individually.
                  </div>
                </button>
              </div>
