...
function FoundationPage() {
  ...
  const save = async () => {
    const { error } = await supabase
      .from("foundation")
      .upsert({ user_id: uid, ...form, updated_at: new Date().toISOString() });
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          The bedrock of your 90 days. Come back here when you forget why you started.
        </p>
      </div>
      ...
      <Card>
        <CardContent className="p-6 space-y-5">
          <Field label="One word for the 90 days" hint="A single word to organize every decision.">
            <Input
              value={form.one_word}
              onChange={(e) => setForm({ ...form, one_word: e.target.value })}
          ...
          <Field label="Dream outcome" hint="What does the finish line look like in vivid detail?">
            <Textarea
              ...
              onChange={(e) => setForm({ ...form, dream_outcome: e.target.value })}
            />
          </Field>
          <Field
            label="Why this matters"
            hint="The deeper reason. The thing that gets you up at 5am."
          >
            <Textarea
              rows={3}
              value={form.why_matters}
              onChange={(e) => setForm({ ...form, why_matters: e.target.value })}
            />
          </Field>
          <Field
            label="Success headline"
            hint="If a newspaper covered your day 90, what would the headline say?"
          >
            <Input
              value={form.success_headline}
              onChange={(e) => setForm({ ...form, success_headline: e.target.value })}
            />
          </Field>
