...
function Dashboard() {
  ...
  const daysLeftOfChallenge = Math.max(
    0,
    CHALLENGE_DAYS - differenceInCalendarDays(new Date(), new Date(start)),
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      const awarded = await maybeAwardBadges(
        uid,
        scoresQ.data!.map((s) => ({ date: s.score_date, score: s.daily_score })),
        day,
      );
      if (awarded.length) {
  ...
  const weekAvg = weekScores.length
    ? Math.round(weekScores.reduce((a, b) => a + b.daily_score, 0) / weekScores.length)
    : 0;
  ...
  for (const e of fin) {
    ...
    if (e.kind === "income") allIn += v;
    else allOut += v;
    if (new Date(e.entry_date) >= monthStart) {
      if (e.kind === "income") mIn += v;
      else mOut += v;
    }
  }
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-widest text-muted-foreground flex items-center gap-1">
            ...
            <span className="text-accent ml-1 font-medium">• {daysLeftOfChallenge} days left</span>
          </div>
          <h1 className="font-display text-4xl md:text-5xl mt-1">
            {greeting()},{" "}
            <span className="gradient-text italic">
              {profileQ.data?.display_name?.split(" ")[0] ?? "friend"}.
            </span>
          </h1>
          {foundationQ.data?.one_word && (
            <p className="mt-2 text-muted-foreground">
              This is the season of{" "}
              <span className="font-medium text-accent">{foundationQ.data.one_word}</span>.
            </p>
          )}
        </div>
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">
              ...
              <span className="text-accent font-medium ml-1">
                ({daysLeftOfChallenge} days remaining)
              </span>
            </span>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Stat
          ...
          label="Today"
          value={`${today?.daily_score ?? 0}`}
          sub={<span className={grade.color}>Grade {grade.grade}</span>}
        />
        <Stat
        ...
        <Stat
          ...
          label="Goals"
          value={`${(goalsQ.data ?? []).length}`}
          sub={`${Math.round((goalsQ.data ?? []).reduce((a, g: any) => a + (g.progress || 0), 0) / Math.max(1, (goalsQ.data ?? []).length))}% avg progress`}
        />
      </div>
      ...
      <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/5 via-card to-accent/5">
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          ...
          <a href="/finances" className="text-xs text-primary hover:underline">
            Open →
          </a>
        </CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-4 text-sm">
          ...
          <div>
            ...
            <div
              className={`font-display text-2xl mt-1 ${mNet >= 0 ? "text-emerald-500" : "text-rose-500"}`}
            >
              ${mNet.toFixed(2)}
            </div>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>Daily score (30 days)</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Life scores - today</CardTitle>
          </CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <RadialBarChart
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {(tasksQ.data ?? []).length === 0 && (
              <p className="text-sm text-muted-foreground">
                Nothing upcoming. Plan your next move in Projects.
              </p>
            )}
            {(tasksQ.data ?? []).map((t) => {
      ...
      <Card>
        ...
        <CardContent className="flex flex-wrap gap-2">
          {(achievementsQ.data ?? []).length === 0 && (
            <p className="text-sm text-muted-foreground">
              Your first badge is waiting - complete day 1.
            </p>
          )}
