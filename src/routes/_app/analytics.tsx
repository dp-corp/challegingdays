...
function AnalyticsPage() {
  ...
  const habitsQ = useQuery({
    queryKey: ["habits", uid],
    queryFn: async () =>
      (await supabase.from("habits").select("*").eq("user_id", uid).eq("active", true)).data ?? [],
  });
  const habitLogsQ = useQuery({
    queryKey: ["habit-logs-90", uid],
    queryFn: async () =>
      (await supabase.from("habit_logs").select("*").eq("user_id", uid).gte("log_date", from))
        .data ?? [],
  });
  ...
  const reviewData = (reviewsQ.data ?? []).map((r) => ({
    ...
    income: Number(r.income_earned ?? 0),
    savings: Number(r.savings_added ?? 0),
    energy: r.energy_level ?? 0,
    exercise: r.exercise_days ?? 0,
  }));

  const today = scores.find((s) => s.score_date === todayISO());
  const avg = scores.length
    ? Math.round(scores.reduce((a, b) => a + b.daily_score, 0) / scores.length)
    : 0;
  const best = scores.reduce(
    (a, b) => (b.daily_score > (a?.daily_score ?? 0) ? b : a),
    null as any,
  );
  ...
  const pieData = statuses
    .map((s) => ({ name: s, value: (tasksQ.data ?? []).filter((t) => t.status === s).length }))
    .filter((d) => d.value > 0);
  const pieColors = [
    "var(--chart-1)",
    "var(--chart-2)",
  ...
  // goal progress
  const goalChart = (goalsQ.data ?? []).map((g) => ({
    name: g.title.slice(0, 18),
    progress: g.progress,
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Stat
          ...
          label="Best day"
          value={best ? `${best.daily_score}` : "-"}
          sub={best ? format(new Date(best.score_date), "MMM d") : undefined}
        />
        <Stat
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Today across areas</CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>When are you sharpest?</CardDescription>
          </CardHeader>
          <CardContent className="h-64">
            <ResponsiveContainer>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Top habit consistency</CardTitle>
            <CardDescription>Last 90 days, completions</CardDescription>
          </CardHeader>
          <CardContent className="h-64">
            {leaders.length === 0 ? (
              <p className="text-sm text-muted-foreground">Log a habit to see this.</p>
            ) : (
              <ResponsiveContainer>
                <BarChart data={leaders} layout="vertical" margin={{ left: 8 }}>
                  <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Weekly income & savings</CardDescription>
          </CardHeader>
          <CardContent className="h-64">
            <ResponsiveContainer>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Energy & exercise</CardTitle>
            <CardDescription>Weekly self-report</CardDescription>
          </CardHeader>
          <CardContent className="h-64">
            <ResponsiveContainer>
      ...
      <Card>
        <CardContent className="p-5 text-sm text-muted-foreground">
          {totalLogs} habit completions logged · {scores.length} days scored ·{" "}
          {(goalsQ.data ?? []).length} active goals
        </CardContent>
      </Card>
    </div>
  );
